...
classes.post("/", requireRole("ADMIN", "TEACHER"), async (c) => {
  ...
  const cls = await classService.create(user.userId, user.role, data);
...
classes.get("/", async (c) => {
  ...
  let teacherId = teacherIdParam ? Number(teacherIdParam) : undefined;
  ...
  const result = await classService.list({ page, limit, search, teacherId, studentId });
